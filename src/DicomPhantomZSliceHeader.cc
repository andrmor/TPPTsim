...
DicomPhantomZSliceHeader::DicomPhantomZSliceHeader(const G4String& fname, const G4String& fpath)
:   fNoVoxelX(0),fNoVoxelY(0),fNoVoxelZ(0),
    fMinX(0),fMinY(0),fMinZ(0),
    fMaxX(0),fMaxY(0),fMaxZ(0),
    fFilename(fname), fPath(fpath),
    fSliceLocation(0)
{
...
//   G4cout << "DicomPhantomZSliceHeader::Dumping Z Slice data to " << fFilename << "..." << G4endl;
  ...
  std::ofstream out;
  char * fileOpen = new char[512];

  std::sprintf(fileOpen,"%s/%s", fPath.c_str(), fFilename.c_str());
